...
document.getElementById("btn1").addEventListener("click", function(){
    ...
    const product = {
        ...
        'information' : function(){ //객체의 기능
            ...
            // 해당 객체의 변수(key)가 아닌 전역 변수가 선택됨
            // 왜? 같은 객체의 변수(key)를 선택하는 방법이 별도 존재헤서
...
document.getElementById("btn2").addEventListener("click",function(){
    ...
    stdList.push(new Student(2, 7, '최길동'));
    stdList.push(new Student(1, 6, '문길동'));
    ...
    for(let std of stdList){
        ...
        // ***********
        // 프로젝트 때 자주 사용! -> 회원, 게시글 전체 얻어오기 (전체 게시글)
        for(let key in std) {
...
function Student(grade, ban, name){
    ...
    // 속성
    this.grade = grade;
    // 매개변수로 전달받은 grade값을 생성되는 객체의 grade에 대입
    this.ban = ban;
    this.name = name;
